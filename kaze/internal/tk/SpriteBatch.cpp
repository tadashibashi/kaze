...
#include <kaze/internal/graphics/Renderer.h>
#include <kaze/internal/graphics/Renderable.h>

#include <filesystem>

KTK_NS_BEGIN

using namespace KGFX_NS_INTERNAL;
...
struct SpriteBatch::Impl
{
    ...
    auto init(const Renderer &graphics) -> Bool
    {
        if (wasInit())
            return True;
        ...
        const auto result = m_renderable.init({
            ...
            .layout = VertexLayout()
                .begin()
                    ...
                    .add(VertexAttrib::Color0, 4, VertexAttribType::Uint8, True)
                .end(),
        });
...
auto SpriteBatch::init(const Renderer &graphics) -> Bool
{
