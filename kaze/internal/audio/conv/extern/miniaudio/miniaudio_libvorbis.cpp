...
#include <kaze/internal/core/io/stream//Rstreamable.h>
...
static int ma_libvorbis_vf_callback__seek(void* pUserData, ogg_int64_t offset, int whence)
{
    ...
    if (result != MA_SUCCESS) {
        KAZE_PUSH_ERR(KAZE_NS_INTERNAL::Error::RuntimeErr, "Failed to seek vorbis file\n");
        return -1;
    }
    return 0;
}
...
{
    KAZE_NS_INTERNAL::Rstreamable *stream = (KAZE_NS_INTERNAL::Rstreamable *)pDecoder->pUserData;
    *pCursor = stream->tell();
    return MA_SUCCESS;
}
