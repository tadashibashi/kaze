#pragma once
#ifndef nova_macros_enum_h_
#define nova_macros_enum_h_
...
#define NOVA_DEFINE_FLAGNAME_CASE(FlagName) \
    case FlagName: return #FlagName;

#define NOVA_DEFINE_FLAGNAME_SWITCH(ParamName, ...) \
    switch (ParamName) { \
        NOVA_MAP(NOVA_DEFINE_FLAGNAME_CASE, __VA_ARGS__) \
        default: return ""; \
    }
#define NOVA_DEFINE_FLAGNAME_GETTER(Class, FuncName, ...) \
    static constexpr const Char *FuncName(Class::Flags_ flag) { \
        NOVA_DEFINE_FLAGNAME_SWITCH(flag, __VA_ARGS__) \
    }

#define NOVA_DEFINE_FLAG(Flag) Flag = 1u << (int)Enum_::Flag

#define NOVA_DEFINE_FLAG_MEMBERS(Name, ...) \
private:                                                                          \
    enum class Enum_ : NOVA_NAMESPACE::Int                                        \
    {                                                                             \
        __VA_ARGS__,                                                              \
        PRIVATE_Enum_CoUnT_                                                       \
    };                                                                            \
public:                                                                           \
    NOVA_NAMESPACE::Uint value;                                                   \
    enum Flags_ : NOVA_NAMESPACE::Uint                                            \
    {                                                                             \
        NOVA_MAP_LIST(NOVA_DEFINE_FLAG, __VA_ARGS__)                              \
    };                                                                            \
    /* NOLINT(*-explicit-constructor) */                                          \
    explicit constexpr Name(Flags_ flag) : value(flag) { }                        \
    explicit constexpr Name(NOVA_NAMESPACE::Uint val = 0) : value(val) { }        \
    Name &operator=(const Name &flags) { value = flags.value; return *this; }     \
    Name &operator=(Flags_ flag) { value = flag; return *this; }                  \
                                                                                  \
    constexpr operator NOVA_NAMESPACE::Uint() const { return value; }                       \
                                                                                            \
    ...
    Name& toggle(Name::Flags_ flag) { value ^= static_cast<NOVA_NAMESPACE::Uint>(flag); return *this; }                \
    ...
    static constexpr NOVA_NAMESPACE::Int count() { return static_cast<int>(Enum_::PRIVATE_Enum_CoUnT_) - 1; }          \
...
#define NOVA_DEFINE_FLAGS(Name, ...) \
    struct Name { \
        NOVA_DEFINE_FLAG_MEMBERS(Name, __VA_ARGS__); \
    }

#define NOVA_DEFINE_FLAGS_WITH_NAMES(Name, ...) \
    struct Name { \
        NOVA_DEFINE_FLAG_MEMBERS(Name, __VA_ARGS__); \
    public: \
        NOVA_DEFINE_FLAGNAME_GETTER(Name, getName, __VA_ARGS__) \
    }
// Define global bitwise operators for a specific type that was previously declared via NOVA_DEFINE_FLAGS
...
#define NOVA_DEFINE_FLAGOPS(Name) \
constexpr Name operator|(Name::Flags_ a, Name::Flags_ b)                       \
{                                                                           \
    return Name(static_cast<NOVA_NAMESPACE::Uint>(a) | static_cast<NOVA_NAMESPACE::Uint>(b));       \
}                                                                           \
constexpr Name operator&(Name::Flags_ a, Name::Flags_ b)                       \
{                                                                           \
    return Name(static_cast<NOVA_NAMESPACE::Uint>(a) & static_cast<NOVA_NAMESPACE::Uint>(b));       \
...
#define NOVA_DEFINE_FLAGOPS_WITH_NAMES(Name) \
...
/// The default type of the flags are `NOVA_NAMESPACE::Uint`, 32-bit unsigned integers.
///
...
/// @code
/// NOVA_FLAGS_LEAN(AbilityFlag, CanJump, CanFly, CanSwim);
///
/// int main()
/// {
...
///             e.g. NOVA_FLAGS_LEAN(Abilities, CanFly, CanJump, CanSwim)
///             Abilities::CanFly = 1
///             Abilities::CanJump = 2
///             Abilities::CanSwim = 4
#define NOVA_FLAGS_LEAN(Name, ...)          \
NOVA_DEFINE_FLAGS(Name, __VA_ARGS__); \
NOVA_DEFINE_FLAGOPS(Name)

#define NOVA_FLAGS(Name, ...) \
    NOVA_DEFINE_FLAGS_WITH_NAMES(Name, __VA_ARGS__); \
    NOVA_DEFINE_FLAGOPS(Name) \
    NOVA_DEFINE_FLAGOPS_WITH_NAMES(Name)

#endif // nova_macros_enum_h_
